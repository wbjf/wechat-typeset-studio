...
三类都可以**插到你指定的位置**，两种定法任选：

- **在右侧成品里点某一段** —— 插到那一段之后。被点的那一段会被描边高亮，点一下就看得见插到哪。
- **在左侧文稿里点一个位置** —— 插到光标处。适合要精确到"某一句话中间"的场景。

两处都没点过就追加到文末（即"直接粘贴一篇文稿就开始排版"的默认路径）。连着插几个会依次往后排，不会都堆在一起。

左栏底部状态条上一直显示着**当前插入点**（`插入点：第 12 行` / `插入点：文末`），插入完成后提示条也会写明落点——不用猜插到哪。

实现上有个细节：右侧是成品视图，段落里的 Markdown 记号已经被渲染掉了，和左侧不是同一套字符，所以定位按**块**换算——解析文稿时记下每一块的字符区间，渲染时写进节点的 `data-from` / `data-off`，点击时反查。这些标记在导出/复制前会被抹掉，不会进公众号，也不占字符预算。

注意一点：点文稿**下方的空白处**，浏览器原生就会把光标放到文末，此时插入点显示为「文末」。想插到中间，请点在文字上。
...
工具条「插入 ▾ → 装饰元件」会直接把这一行写到你的文稿里，改文字即可。写到哪由插入点决定：在右侧成品里点某一段就插到那一段后面，或在左侧文稿里点一个位置。
...
109 项断言，全部通过则退出码为 0；明细写到 `test/verification-report.txt`，截图写到 `test/artifacts/`。
...
- **插入点两条路径都验**：左栏光标置于文档中段时插入落在光标处、连着插两个依次往后排、没碰过左侧时追加到文末；**在右侧预览里点某一段**时插入点换算成该段末尾、该段被描边高亮、内容确实插在那一块之后（比较插入前后的首个差异位置）、回到左栏后高亮取消并改跟左栏光标
- **预览定位标记不外泄**：导出 HTML 里不含 `data-off` / `data-from` / `blk-target`；每块的源文区间非零长度且单调递增（单行块如果先 push 再 `i++` 会得到零长度区间，这条断言就是防这个）
- **插入点指示不压垮布局**：左栏拖到最窄（280px）时底部状态条仍为单行 35px、指示不被截断，放不下的「阅读时长」整体让位且不撑出横向滚动条
- 全流程无未捕获 JS 错误
